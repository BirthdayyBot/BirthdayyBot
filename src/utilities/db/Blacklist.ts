...
export class Blacklist extends Utility {
	...
	public create = {
		BlacklistEntry: (guildId: string, userId: string) => this.prisma.blacklist.create({ data: { guildId, userId } })
	};

	public delete = {
		BlacklistEntry: (guildId: string, userId: string) => this.prisma.blacklist.delete({ where: { userId_guildId: { userId, guildId } } })
	};
