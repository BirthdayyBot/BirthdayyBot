...
export class Guild extends Utility {
	public get = {
		...
		GuildLanguage: async (guildId: string) => this.prisma.guild.findUnique({ where: { guildId }, select: { guildId: true, language: true } }),
		GuildPremium: async (guildId: string) => this.prisma.guild.findUnique({ where: { guildId }, select: { guildId: true, premium: true } }),
		GuildDisabled: async (guildId: string) => this.prisma.guild.findUnique({ where: { guildId }, select: { guildId: true, disabled: true } }),
	...
	public set = {
		...
		Timezone: async (guildId: string, timezone: number | undefined) => this.prisma.guild.update({ where: { guildId }, data: { timezone } }),
		Language: async (guildId: string, language: string | undefined) => this.prisma.guild.update({ where: { guildId }, data: { language } }),
		...
		Premium: async (guildId: string, premium: boolean | undefined) => this.prisma.guild.update({ where: { guildId }, data: { premium } })
	};
