...
import { BOT_ADMIN_LOG, DEFAULT_ANNOUNCEMENT_MESSAGE } from '#root/helpers/provide/environment';
...
export class Guild extends Utility {
	public get = {
		...
		GuildsByTimezone: (guildIds: string[], timezone: number) => this.prisma.guild.findMany({ where: { guildId: { in: guildIds }, timezone } }),
		GuildsDisabled: (disabled = true) => this.prisma.guild.findMany({ where: { disabled } }),
		GuildLanguage: (guildId: string) => this.prisma.guild.findUnique({ where: { guildId }, select: { guildId: true, language: true } }),
		GuildPremium: (guildId: string) => this.prisma.guild.findUnique({ where: { guildId }, select: { guildId: true, premium: true } }),
		PremiumGuilds: () => this.prisma.guild.findMany({ where: { premium: true } }),
		GuildDisabled: (guildId: string) => this.prisma.guild.findUnique({ where: { guildId }, select: { guildId: true, disabled: true } }),
		...
		GuildTimezone: (guildId: string) => this.prisma.guild.findUnique({ where: { guildId }, select: { guildId: true, timezone: true } })
	};
	...
	public set = {
		...
		LogChannel: (guildId: string, channelID: string) => this.prisma.guild.update({ where: { guildId }, data: { logChannel: channelID } }),
		Timezone: (guildId: string, timezone: number) => this.prisma.guild.update({ where: { guildId }, data: { timezone } }),
		Language: (guildId: string, language: string) => this.prisma.guild.update({ where: { guildId }, data: { language } }),
		BirthdayRole: (guildId: string, roleID: string) => this.prisma.guild.update({ where: { guildId }, data: { birthdayRole: roleID } }),
		BirthdayPingRole: (guildId: string, roleID: string) => this.prisma.guild.update({ where: { guildId }, data: { birthdayPingRole: roleID } }),
		Premium: (guildId: string, premium: boolean) => this.prisma.guild.update({ where: { guildId }, data: { premium } })
	};
	...
	public reset = {
		AnnouncementChannel: (guildId: Snowflake) => this.prisma.guild.update({ where: { guildId }, data: { announcementChannel: null } }),
		...
		OverviewChannel: (guildId: Snowflake) => this.prisma.guild.update({ where: { guildId }, data: { overviewChannel: null } }),
		OverviewMessage: (guildId: Snowflake) => this.prisma.guild.update({ where: { guildId }, data: { overviewMessage: null } }),
		LogChannel: (guildId: Snowflake) => this.prisma.guild.update({ where: { guildId }, data: { logChannel: null } }),
		...
		BirthdayRole: (guildId: Snowflake) => this.prisma.guild.update({ where: { guildId }, data: { birthdayRole: null } }),
		BirthdayPingRole: (guildId: Snowflake) => this.prisma.guild.update({ where: { guildId }, data: { birthdayPingRole: null } })
	};
