...
@RegisterSubCommand('config', (builder) =>
	builder
		...
		.addStringOption((option) =>
			option.setName('message').setDescription('{MENTION}, {USERNAME}, {DISCRIMINATOR}, {NEW_LINE}, {SERVERNAME}').setRequired(true)
		)
)
export class AnnouncementMessageCommand extends Command {
	public override async chatInputRun(interaction: Command.ChatInputInteraction<'cached'>) {
		...
		if (message.length > EmbedLimits.MaximumDescriptionLength - 500) {
			return reply(interaction, interactionProblem('The message you provided is too long. Please try again with a shorter message.'));
		}
