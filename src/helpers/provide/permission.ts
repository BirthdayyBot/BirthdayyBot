...
export async function hasBotGuildPermissions({ interaction, permissions }: Omit<Options, 'channel' | 'user'>): Promise<boolean> {
	const bot = await interaction.guild.members.fetchMe();
	return bot.permissions.has(permissions);
}

export async function hasBotChannelPermissions({ interaction, channel, permissions }: Omit<Options, 'user'>): Promise<boolean> {
	const bot = await interaction.guild.members.fetchMe();
...
export async function hasUserChannelPermissions({ interaction, user, channel, permissions }: Options): Promise<boolean> {
	const member = await interaction.guild.members.fetch(user);
	const channelData = await container.client.channels.fetch(typeof channel === 'string' ? channel : channel.id);
	if (!channelData || !channelData.isTextBased() || channelData.isDMBased()) return false;
